// ============================================================================
// PROBABILISTIC REVERSAL LEARNING TASK - TASK LOGIC
// ============================================================================
// This file contains all JavaScript logic for the PRLT task
// Extracted from PRLT Flexible 6.0.html
...
// ========================================================================
// SECCIÓN 3: DECKMANAGER CLASS FOR DEN OUDEN STYLE RANDOMIZATION
// ========================================================================
// Implements 10-card deck system with constraints to prevent misleading patterns
...
class DeckManager {
  ...
  generateDeck(prob) {
    ...
    let deck = Array(wins).fill(1).concat(Array(losses).fill(0));
    
    // Shuffle and ensure max 3 consecutive
    let valid = false;
    while (!valid) {
      this.shuffle(deck);
      valid = !this.hasMoreThanThreeConsecutive(deck);
    }
    return deck;
  }
  ...
  getFeedback(side, prob, forceCongruent) {
    if (this.decks[side].length === 0) {
      this.decks[side] = this.generateDeck(prob);
    }

    if (forceCongruent) {
      // High prob (>0.5) should win (1), Low prob (<0.5) should lose (0)
      const expected = prob > 0.5 ? 1 : 0;
      // Try to find expected in deck
      const idx = this.decks[side].indexOf(expected);
      if (idx !== -1) {
        this.decks[side].splice(idx, 1);
        return expected;
      } else {
        // Fallback: regenerate deck and use first matching card
        this.decks[side] = this.generateDeck(prob);
        return expected;
      }
    }
    return this.decks[side].shift();
  }
}

// ========================================================================
// SECCIÓN 4: FUNCIONES DE GENERACIÓN DE URNAS DE FEEDBACK
// ========================================================================
...
// veraz vs. engañoso, manteniendo urnas separadas por corrección de respuesta.
      ...
      /**
       * Genera un deck pseudo-aleatorio según el método de Den Ouden et al.
       * Crea un deck con proporciones exactas y valida que no haya rachas > 3 consecutivas.
       * 
       * @param {number} numTrials - Número de trials para el deck
       * @param {number} probability - Probabilidad de feedback veraz (e.g., 0.7 para 70%)
       * @returns {Array<boolean>} Deck con valores true (feedback veraz) y false (engañoso)
       */
      function generateDenOudenDeck(numTrials, probability) {
        ...
        while (attempts < maxAttempts) {
          ...
          // 1. Crear deck con proporciones exactas
          const deck = [];
          const truthfulCount = Math.round(numTrials * probability);
          const misleadingCount = numTrials - truthfulCount;
          
          for (let i = 0; i < truthfulCount; i++) deck.push(true);
          for (let i = 0; i < misleadingCount; i++) deck.push(false);
          
          // 2. Shuffle usando Fisher-Yates
          for (let i = deck.length - 1; i > 0; i--) {
            const j = Math.floor(Math.random() * (i + 1));
            [deck[i], deck[j]] = [deck[j], deck[i]];
          }
          
          // 3. Validar que no haya rachas > 3
          let isValid = true;
          for (let i = 0; i <= deck.length - 4; i++) {
            if (deck[i] === deck[i+1] && deck[i] === deck[i+2] && deck[i] === deck[i+3]) {
              isValid = false;
              break;
            }
          }
          ...
          if (isValid) {
            console.log(`✓ Deck Den Ouden generado en ${attempts} intentos`);
            return deck;
          }
        }
        
        console.warn(`⚠️ No se pudo generar deck válido en ${maxAttempts} intentos. Usando deck sin validar.`);
        // Fallback: devolver último intento
        const deck = [];
        const truthfulCount = Math.round(numTrials * probability);
        for (let i = 0; i < truthfulCount; i++) deck.push(true);
        for (let i = 0; i < numTrials - truthfulCount; i++) deck.push(false);
        for (let i = deck.length - 1; i > 0; i--) {
          const j = Math.floor(Math.random() * (i + 1));
          [deck[i], deck[j]] = [deck[j], deck[i]];
        }
      ...
      /**
       * Obtiene feedback según el método de randomización seleccionado.
       * @param {boolean} trueCorrect - Si la elección del paciente fue correcta
       * @returns {boolean} Si el feedback mostrado será positivo (true) o negativo (false)
       */
      function getFeedback(trueCorrect) {
        if (randomizationMethod === 'pseudo') {
          // Método Den Ouden: usar decks pre-generados
          const deck = trueCorrect ? denOudenDeckCorrect : denOudenDeckIncorrect;
          
          if (deck.length === 0) {
            console.error('⚠️ Deck vacío! Regenerando...');
            const newDeck = generateDenOudenDeck(100, feedbackProbability);
            if (trueCorrect) {
              denOudenDeckCorrect.push(...newDeck);
            } else {
              denOudenDeckIncorrect.push(...newDeck);
            }
          }
          
          const giveTruthful = trueCorrect ? denOudenDeckCorrect.shift() : denOudenDeckIncorrect.shift();
          return giveTruthful ? trueCorrect : !trueCorrect;
        } else {
          // Método 'pure': usar urnas tradicionales
     ...
     * Genera los estímulos según la configuración y muestra la primera cruz de fijación.
     */
    function startMainTask() {
      
      // Generar estímulos reales
      const stimulusType = document.getElementById('stimulusTypeSelect').value;
      if (stimulusType === 'genios') {
    ...
    function processChoice(choice) {
      ...
      setTimeout(() => {
        ...
        if (shouldReverse) {
          ...
          // Reset decks on reversal if using deck mode
          if (randomizationMode === 'deck' && deckManager) {
            deckManager.resetDecks();
            justReversed = true; // Mark that we just reversed
          }
        ...
        // Generate feedback based on randomization mode
        let feedbackCorrect;
        if (randomizationMode === 'deck' && deckManager) {
          // Deck mode: use DeckManager
          // Determine if we need to force congruent feedback (first trial or just reversed)
          const forceCongruent = isFirstTrial || justReversed;
          
          // Determine side based on choice
          const side = choice === 'A' ? 'left' : 'right';
          
          // Determine probability for this choice
          const prob = trueCorrect ? feedbackProbability : (1 - feedbackProbability);
          
          // Get feedback from deck
          const deckFeedback = deckManager.getFeedback(side, prob, forceCongruent);
          
          // deckFeedback is 1 for win, 0 for loss
          // If choice is correct and deckFeedback is 1, or choice is incorrect and deckFeedback is 0
          // then feedback is "truthful" (congruent)
          // We need to translate this to feedbackCorrect (positive/negative outcome)
          if (trueCorrect) {
            // If choice is correct, deckFeedback=1 means positive, 0 means negative
            feedbackCorrect = deckFeedback === 1;
          } else {
            // If choice is incorrect, deckFeedback=0 means negative, 1 means positive (misleading)
            feedbackCorrect = deckFeedback === 1;
          }
          
          // Reset flags after first trial
          if (isFirstTrial) {
            isFirstTrial = false;
          }
          if (justReversed) {
        ...
        } else if (randomizationMethod === 'pseudo') {
          // Den Ouden pseudo-random mode: use pre-generated decks
          feedbackCorrect = getFeedback(trueCorrect);
        } else {
          // Random mode (classic): use existing urn system
...
/// final bloqeu showresults
    ...
    function downloadCsv() {
      ...
      const meta = {
        ...
        meta_randomization_method: randomizationMethod || 'pure',  // ⭐ NUEVO: método de randomización
...
//comprobación de qu se ha selecionado la correcta)
      ...
      // ⭐ INITIALIZE DECK MANAGER AND RANDOMIZATION MODE
      randomizationMode = document.getElementById('randomizationModeSelect').value || 'deck';
      
      // ⭐ ALSO CHECK FOR NEW RANDOMIZATION METHOD SELECTOR
      const randomMethodSelect = document.getElementById('randomizationMethodSelect');
      if (randomMethodSelect) {
        randomizationMethod = randomMethodSelect.value || 'pure';
      }
      
      // Initialize based on randomization mode
      ...
      // ⭐ INITIALIZE DEN OUDEN DECKS IF PSEUDO METHOD SELECTED
      if (randomizationMethod === 'pseudo') {
        // Generate pre-randomized decks for correct and incorrect responses
        denOudenDeckCorrect = generateDenOudenDeck(maxTrials, feedbackProbability);
        denOudenDeckIncorrect = generateDenOudenDeck(maxTrials, feedbackProbability);
        console.log(`🎯 Den Ouden decks initialized (${maxTrials} trials, ${(feedbackProbability*100).toFixed(0)}% truthful)`);
        console.log(`   - Correct deck: ${denOudenDeckCorrect.filter(x=>x).length} truthful, ${denOudenDeckCorrect.filter(x=>!x).length} misleading`);
        console.log(`   - Incorrect deck: ${denOudenDeckIncorrect.filter(x=>x).length} truthful, ${denOudenDeckIncorrect.filter(x=>!x).length} misleading`);
      }
      
      // Initialize first trial flags
      isFirstTrial = true;
      justReversed = false;

      // Generate stimuli directly (no practice block)
      const stimulusType = document.getElementById('stimulusTypeSelect').value;
      if (stimulusType === 'genios') {
        stimuliDiv.innerHTML = `
          <div class="stimulus" id="A">
...
// ============================================================================
// ADDITIONAL FUNCTIONALITY - Preset feedbackProbability + Debug + Exports
// ============================================================================
